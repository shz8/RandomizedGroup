...
module.exports = {
  ...
  dynamicRandom(data) {
    ...
    let groups = [];
    for (let idx = 0; idx < data.groupNum; idx++) {
      groups.push([])
    }
    let groupsFields = [];
    let groupsTotal = [0, 0];
    ...
    for (let midx = 0; midx < samplesLength; midx++) {
      ...
      //获取要存入的分组索引
      let gidx = isRangeMethod ? this.dyncAddSampleRangeMethod(groupsFields, midFields, midx, samplesLength, groupsTotal) : this.dyncAddSamplesStandardDev(groupsFields, midFields, midx, samplesLength, groupsTotal)
  ...
  /*标准差（适用于分>=2组）
  */
  dyncAddSamplesStandardDev(groupsFields, midFields, sampleIdx, samplesLength, groupsTotal) {
    let gidx = -1
