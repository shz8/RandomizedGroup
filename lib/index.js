...
module.exports = {
  ...
  /*简单随机
   *data.groups 分组列表，如： ['G001', 'G002', 'G003']
   
   *返回：分组号
   */
  simpleRandomIncrement(data) {
    if (!data) return { msg: "参数不能为空！" };
    if (!(data.groups && data.groups instanceof Array)) {
      return { msg: `参数groups缺失或不为数组！` };
    }
    let samples = data.samples || []
    if (!data.samples instanceof Array) {
      return { msg: `参数samples必须为数组！` };
    }
    if(!data.incrementSample) {
      return { msg: `参数incrementSample缺失！` };
    }
    let minLen = -1
    let groups = data.groups.map(d=>{
      let len = samples.filter(s=>s.groupNO == d).length
      minLen = minLen > len || minLen==-1?len:minLen
      return {groupNO:d,len}}
    )
    //取出样本最少的分组
    let minGroups = groups.filter(s=>s.len == minLen);    
    //return minGroups[utils.random(minGroups.length)].groupNO
    //在样本最少的所有分组中随机选一组
    data.incrementSample.groupNO = minGroups[utils.random(minGroups.length)].groupNO
    return  data.incrementSample
  },
  /*区组随机
   *data.groups 分组列表，如： ['G001', 'G002', 'G003']
   ...
   *返回：分组号
   */
  blockRandomIncrement(data) {
    //按加入时间顺序分区，再随机分组。
    return this.simpleRandomIncrement(data)
   },
  /*区组随机
   *data.samples 样本
   *data.groupNum 分组数
  ...
  dynamicRandomIncrement(data) {
    //{ dyncFieds: index.clone(dyncFieds), samples, incrementSample,groups: sampleGroups,  p}
    if (!data) return { msg: "参数不能为空！" };

    let isRangeMethod = false
    if (!(data.groups && data.groups instanceof Array)) {
      return { msg: `参数groups缺失或不为数组！` };
    }
    data.groupNum = data.groups.length;
    if (!data.incrementSample) {
      return { msg: `参数incrementSample（新增样本）缺失！` };
    }
    data.samples = data.samples || []
    data.samples.push(data.incrementSample)

    data.from = 'dynamicRandomIncrement'

    data.groupIndex = {}
    for (let idx = 0; idx < data.groups.length; idx++) {
      data.groupIndex[data.groups[idx]] = idx
    }
    console.log('groups', data.groups)
    return this.dynamicRandomNoGroupProportion(data)
  },
  addGroupProportionInfo(groupsFields, data) {
    if (data.from == 'dynamicRandomIncrement' && data.groupProportion && data.groupProportion instanceof Array) {
      let groupProportion = data.groupProportion
      for (let idx = 0; idx < groupsFields.length; idx++) {
        let p = groupProportion[idx] || 1
        groupsFields[idx].proportion = (p < 1 || p > 5) ? 1 : p
      }
    }
  },
  /*动态随机
  ...
  dynamicRandomNoGroupProportion(data) {
    ...
    */
    ...
    this.addGroupProportionInfo(groupsFields, data)
    ...
    for (let midx = 0; midx < samplesLength; midx++) {
      ...
      //如果是增量添加样本，则获取样本原来的分组
      let oldGroupIdx = data.groupIndex ? data.groupIndex[samples[midx].groupNO] : -1
      if (data.groups && data.groups.length > 0) {
        if (midx < (samplesLength - 1) && oldGroupIdx < 0)
          return { msg: `样本的分组${samples[midx].groupNO}不在参数groups数组中！` };
        if (midx == (samplesLength - 1))
          samples[midx].groupNO = ''
      }
      //获取要存入的分组索引
      let gidx = isRangeMethod ? this.dyncAddSampleRangeMethod(groupsFields, midFields, midx, samplesLength, groupsTotal) :
        this.dyncAddSamplesStandardDev(groupsFields, midFields, midx, samplesLength, groupsTotal, p, oldGroupIdx)
      ...
      if (gidx > -1 && gidx < groupsLength) {
        ...
        if (data.groups && data.groups.length > 0) {
          if (midx == (samplesLength - 1)) {
            samples[midx].groupNO = data.groups[gidx]
            //test
            this.printTotalTest(midFields, groupsFields, groupsLength, groups)
            //test
            return { sample: samples[midx] }
          }
        }
      }
    }
  ...
  printTotalTest(midFields, groupsFields, groupsLength, groups) {
    for (let idx = 0; idx < midFields.length; idx++) {
    ...
    for (let idx = 0; idx < groups.length; idx++) {
      console.log(`groupidx=${idx},total=${groups[idx].length}`)
    }
  },
  /*标准差（适用于分>=2组）
  */
  dyncAddSamplesStandardDev(groupsFields, midFields, sampleIdx, samplesLength, groupsTotal, p, oldGroupIdx) {
    let gidx = -1
    if (sampleIdx == 0) gidx = utils.random(groupsFields.length)
    if (oldGroupIdx > -1) gidx = oldGroupIdx
    if (gidx < 0) {
      let arrVal = []
      for (let idx = 0; idx < midFields.length; idx++) {
        for (let sidx = 0; sidx < midFields[idx].calArr.length; sidx++) {
          if (midFields[idx].calArr[sidx].total == 1) {
            ...
            for (let vidx = 0; vidx < groupsFields.length; vidx++) {
              let standardDevArr = []
              for (let gi = 0; gi < groupsFields.length; gi++) {
                let total = groupsFields[gi][idx].calArr[sidx].total
                total = total + (vidx == gi ? 1 : 0)
                if (groupsFields[gi].proportion > 0) {
                  for (let pi = 0; pi < groupsFields[gi].proportion; pi++) {
                    standardDevArr.push(total / groupsFields[gi].proportion)
                  }
                }
                else
                  standardDevArr.push(total)
              }
      ...
      if (p == 1) {//偏倚率p=1
        ...
        gidx = arrVal[this.getGroupIdx(p, groupsFields.length)].idx //暂不考虑组间比例对偏颇率的影响
