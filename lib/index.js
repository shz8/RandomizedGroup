...
module.exports = {
  ...
  dynamicRandom(data) {
    ...
    groupsP = []
    groupsP.push(parseInt(p * 100))
    let pk = parseInt((100 - groupsP[0]) / (groupNum - 1))
    let p1 = 100 - (groupNum - 2) * pk
    groupsP.push(p1)
    for (let idx = 0; idx < groupNum - 2; idx++) {
      groupsP.push(groupsP[groupsP.length - 1] + pk)
    }
    let groups = [];
    console.log(groupsP)
    ...
    */
    ...
    for (let midx = 0; midx < samplesLength; midx++) {
      ...
      //获取要存入的分组索引
      let gidx = isRangeMethod ? this.dyncAddSampleRangeMethod(groupsFields, midFields, midx, samplesLength, groupsTotal) : this.dyncAddSamplesStandardDev(groupsFields, midFields, midx, samplesLength, groupsTotal, groupsP)
  ...
  /*标准差（适用于分>=2组）
  */
  dyncAddSamplesStandardDev(groupsFields, midFields, sampleIdx, samplesLength, groupsTotal, groupsP) {
    let gidx = -1
    ...
    if (gidx < 0) {
      ...
      for (let idx = 0; idx < midFields.length; idx++) {
        for (let sidx = 0; sidx < midFields[idx].calArr.length; sidx++) {
          if (midFields[idx].calArr[sidx].total == 1) {
            let standardDevArr = []
            for (let gidx1 = 0; gidx1 < groupsFields.length; gidx1++) {
              standardDevArr.push(groupsFields[gidx1][idx].calArr[sidx].total)
            }
            for (let vidx = 0; vidx < arrVal.length; vidx++) {
              standardDevArr[vidx]++; //当前因素水平+1，计算标准差
              arrVal[vidx] += numbers.standardDev(standardDevArr) * (midFields[idx].weight || 1)
      ...
      if (groupsP[0] == 100) {//偏倚率p=1
        ...
        //console.log(minVal)
        for (let vidx = 0; vidx < arrVal.length; vidx++) {
          if (arrVal[vidx] == minVal) {
            if (minGroupTotal > groupsTotal[vidx])//组内最小样本数
              minGroupTotal = groupsTotal[vidx]
            gidxs.push({ idx: vidx, total: groupsTotal[vidx] })
          }
        }
        if (gidxs.length < 1)
          gidx = utils.random(groupsFields.length)
        else if (gidxs.length == 1)
          gidx = gidxs[0].idx
        else {
          for (let idx = gidxs.length - 1; idx > -1; idx--) {
            if (gidxs[idx].total != minGroupTotal) {
              gidxs.slice(idx, 1)//删除不是样本数最小的组
            }
          }
          if (gidxs.length < 1)//应该不会出现
          {
            throw Error('最小值有多个时，取样本数最小分组出错！')
            gidx = utils.random(groupsFields.length)
          }
          gidx = gidxs[utils.random(gidxs.length)].idx //从>=1个组里随机选一个
        }
      }
      else {
        for (let idx = 0; idx < arrVal.length; idx++) {
          arrVal[idx] = { idx, val: arrVal[idx] }
        }
        arrVal = arrVal.sort((a, b) => { return a.val - b.val })
        
        let randomIdx = utils.random(100);
        if (randomIdx <= groupsP[0])
          gidx = arrVal[0].idx
        else {
          for (let idx = 1; idx < arrVal.length; idx++) {
            if (randomIdx > groupsP[idx-1] && randomIdx <= groupsP[idx]) {
              gidx = arrVal[idx].idx
              break
            }
          }
